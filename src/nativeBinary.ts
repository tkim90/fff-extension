...
export function getBundledSidecarPath(extensionUri: vscode.Uri): string {
	...
	if (!PLATFORM_IDS.has(platformId)) {
		throw new Error(
			`Fast Fuzzy Finder supports only darwin-arm64, darwin-x64, linux-x64, and win32-x64. Current platform: ${platformId}.`
		);
	}
	...
	if (!fs.existsSync(binaryPath)) {
		throw new Error(
			`Fast Fuzzy Finder could not find the bundled fff sidecar at ${binaryPath}. Run \`npm run build:native\` before launching from source, or install a packaged build for your platform.`
		);
	}
